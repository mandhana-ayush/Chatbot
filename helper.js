...
const adduser = (data) => {
  ...
  var sameuserobjectpresent = users.find(
    (x) =>
      JSON.stringify(x) ===
      JSON.stringify({ usersocketid: usersocketid, name: name, room: room })
  );
...
const addgroupdata = (data) => {
  ...
  var messages = [];
  var isgroupalreadypresent = groups.filter(
    (x) => x.socketroomid === socketroomid
  );
